    ...
    <div>
      <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-neutral-50 relative transition-all">
        ...
        <div className="hidden sm:flex items-center gap-8">
         ...
         {!user ? (<button onClick={()=> setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-green-500 hover:bg-green-600 transition text-neutral-bg-neutral-50 rounded-full">
            Login
          </button>):
          (
            <div className="relative group ">
             <img className="w-10" 
             src="https://tse4.mm.bing.net/th?id=OIP.hGSCbXlcOjL_9mmzerqAbQHaHa&pid=Api&P=0&h=180" alt="" />
             <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40" >
                <li onClick={()=> navigate("my-orders")} className="p-1.5 pl-3 hover:bg-green-400 cursor-pointer">My Orders</li>
                <li onClick={logout} className="p-1.5 pl-3 hover:bg-green-400 cursor-pointer">Logout</li>
             </ul>
            </div>
          )}
        </div>
